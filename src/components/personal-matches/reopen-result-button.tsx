 ...
 * 되돌리면 양측(복식이면 참가자 전원) 기록이 미확정으로 돌아가 매칭 룸의 「지금 할 일」에 다시 뜨고,
            ...
            <Dialog open={d.open} onOpenChange={d.setOpen}>
                <DialogContent showCloseButton={false}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {description} · 양쪽 기록이 미확정으로 돌아가고 매칭 룸에서 다시 입력합니다.
                        </DialogDescription>
                    </DialogHeader>

                    <form onSubmit={submit} className="space-y-4">
                        <div>
                            <label htmlFor="reopen-reason" className={MATCH_FORM_LABEL}>정정 사유 (선택, {reason.length}/{REASON_MAX})</label>
                            <input
                                id="reopen-reason"
                                value={reason}
