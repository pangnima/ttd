        ...
        run(
            ...
            s.roomId
                ...
                : initialId && s.initialHasResult
                    ? '/me/personal-matches'
                    // 폼은 세트를 받지 않으므로 신규는 언제나 결과 미입력 — 「승인 요청 › 경기 결과 확정」에 뜬다.
 ...
 * 있으므로 「승인 요청 › 경기 결과 확정」이다. 한쪽으로 고정하면 방금 만든 일정이 없는 화면이 나온다.
 */
function rotationLanding(pool: { player: { userId?: string } }[]): string {
    return pool.some((row) => !!row.player.userId) ? hubTabHref('waiting') : hubTabHref('result')
}
