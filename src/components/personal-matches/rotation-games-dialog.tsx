 ...
 */
export function RotationGamesDialog({ open, onOpenChange, session, pool, isRoomSession, picker, onSubmit, isPending, error, poolAdmin }: Props) {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
                ...
                <RotationGamesPanel pool={pool} isRoomSession={isRoomSession} picker={picker} onSubmit={onSubmit} isPending={isPending} error={error} poolAdmin={poolAdmin} />
            </DialogContent>
        </Dialog>
    )
}
