...
export function AdDeuceToggle<T extends string>({ label, options, value, onChange }: Props<T>) {
    return (
        <div>
            <p className="text-caption text-muted-foreground mb-1">{label} 유저 선택</p>
                ...
                {options.map((o) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center justify-between gap-1 px-2.5 py-2 text-body2 rounded-md border transition-colors ${
                                isAd
                                    ? 'border-spot-solid bg-spot-solid text-spot-foreground font-semibold'
                                    : 'border-border text-muted-foreground hover:border-input hover:text-foreground'
                            }`}
                        >
                            <span className="truncate">{o.label}</span>
                            <span className="text-micro shrink-0">{side}</span>
                        </button>
                    )
                })}
            </div>
