...
import { disputedGroupTotals } from '@/lib/match-requests/hub-totals'
 ...
 * 「승인 요청 · 결과 확인 대기」로 조용히 옮겨갔다 — 이의를 낸 사람은 자기가 시작한 분쟁의 결말을
 * 추적할 수 없었다. 이제 확정될 때까지 여기 머물고, 재입력된 결과도 이 화면에서 확인·승인한다.
 *
 * 이 탭은 전부 결과 축이므로(참여 요청이 없다) 안에서는 **차례 축**으로 묶는다 —
 * 다른 두 탭이 차례로 갈린 자리인 것과 대칭이다. 규칙은 하나다:
...
export function DisputedPanel({ queue }: Props) {
    ...
    const groups = disputedGroupTotals(queue.counts)

    return (
        <>
            <HubSectionGroup title="지금 처리할 것" count={groups.myTurn}>
                <PendingMatchSection
                    title="재입력된 결과 확인"
            ...
            <HubSectionGroup title="응답 대기 중" count={groups.waiting}>
                <PendingMatchSection
                    title="내가 이의 제기함"
