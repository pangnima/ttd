...
/**
 * 대진을 읽기 전에 알아야 할 것들 — 이어붙임 안내 · 생성 경고 · 저장 실패.
 *
 * **미리보기 위**에 둔다. "왜 이런 대진이 나왔나"(성별 구성을 못 맞췄다, 인원이 모자라 중단했다)는
 * 결과보다 먼저 읽혀야 하는데, 목록 맨 아래에 있으면 스크롤 끝까지 가야 보인다.
 */
export function RoomLineupNotices({ existingGames, warnings, error }: Props) {
    if (existingGames === 0 && warnings.length === 0 && !error) return null
