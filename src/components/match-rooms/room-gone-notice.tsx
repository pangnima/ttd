...
import { CARD_BASE, TYPO, CTA_LINK } from '@/lib/dashboard/tokens'
 ...
 * 내려간 방의 확정 결과는 room_id만 끊긴 채(0048 set null) 내 경기 결과에 그대로 남는다.
 */
        ...
        <PageContainer>
            ...
            <div className={`${CARD_BASE} px-4 py-3 space-y-3`}>
                <p className={`${TYPO.body2} break-keep`}>
                    호스트가 {NAV_LABEL.matchRooms}에서 내렸거나 존재하지 않는 매칭입니다.
                    이미 확정된 결과는 {NAV_LABEL.myRecords}에 그대로 남아 있습니다.
                </p>
                <Link
                    href="/match-rooms"
                    className={CTA_LINK}
                >
                    {NAV_LABEL.matchRooms}로 돌아가기
                </Link>
            </div>
        </PageContainer>
    )
