        ...
        <div className={`${CARD_BASE} p-4 space-y-3`}>
            <div className="flex items-center justify-between">
                ...
                <span className="text-3xl font-bold text-foreground">
                    {winRateLabel}
                    {!masked && winRateLabel !== '-' && <span className="text-base font-normal text-foreground/75 ml-0.5">승률</span>}
                    {masked && <span className="text-base font-normal text-foreground/75 ml-0.5">승률</span>}
                </span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                    <p className="text-2xl font-bold text-foreground">{masked ? '*' : stats.wins}</p>
                    <p className="text-sm text-foreground/75">승</p>
                </div>
                <div>
                    <p className="text-2xl font-bold text-foreground">{masked ? '*' : stats.losses}</p>
                    <p className="text-sm text-foreground/75">패</p>
                </div>
                <div>
