...
import { splitRoomsByDate, todayIsoKst } from '@/lib/match-rooms/split'
...
/** 경기 리스트 — 리스트에 노출된 경기(방) 전체. 예정(오늘 이후, 가까운 순) / 지난 경기(최근순) 탭 */
export default async function MatchRoomsPage({ searchParams }: Props) {
    ...
    const { upcoming, past } = splitRoomsByDate(rooms, todayIsoKst())
