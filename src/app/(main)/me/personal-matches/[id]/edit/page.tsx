...
import { fetchRoomParticipantCandidates, isRoomClosed } from '@/lib/queries/match-rooms'
...
export default async function EditPersonalMatchPage({ params }: Props) {
    ...
    // 호스트가 마감한 방(0083)의 기록은 소유자도 고칠 수 없다 — 액션·정책이 막지만 폼을 그리기 전에 방으로 돌려보낸다
    if (match.roomId && await isRoomClosed(match.roomId)) redirect(`/match-rooms/${match.roomId}`)
