...
import { buildRoomGameContext, canViewerAddRoomGame, type RoomGameContext } from '@/lib/match-rooms/room-context'
...
/**
 * 방 게임 추가 컨텍스트(0048·0049) — ?room=<id>로 열면 방에 참가한 회원 누구나 게임을 붙일 수 있다.
 * 참가자가 아니거나 게임을 붙일 수 없는 방(수락 전 확인 요청·미확정 로테이션)이면 방 상세로 돌려보낸다.
 */
async function loadRoomContext(roomId: string, userId: string): Promise<RoomGameContext> {
    ...
    if (!detail || !canViewerAddRoomGame(detail, userId)) redirect(`/match-rooms/${roomId}`)
    const participants = await fetchRoomParticipantCandidates(roomId, userId)
    return buildRoomGameContext(detail, participants, userId)
}
