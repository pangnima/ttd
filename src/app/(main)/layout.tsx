...
import { fetchNotificationInbox } from '@/lib/queries/notifications'
...
export default async function MainLayout({
    ...
    // 알림 받은 편지함(Week 71)은 여기서 한 번 — 페이지마다 쿼리 2문(안 읽은 수·최근 10건)이 추가되는 전부다
    const [queue, inbox] = user
        ? await Promise.all([fetchRoomQueue(user.id), fetchNotificationInbox(user.id)])
        : [null, null]
        ...
        <SidebarProvider>
            <div className="flex h-dvh bg-background">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                    <Header userDisplay={userDisplay} userId={user?.id ?? null} myTurnCount={myTurnCount} inbox={inbox} />
