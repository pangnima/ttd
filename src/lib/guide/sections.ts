...
const em = (s: string) => `**${s}**`
const TURN_PILLS = `${em(ROOM_TURN_PILL.enterResult)}·${em(ROOM_TURN_PILL.confirmResult)}`
...
const FLOW: GuideSection = {
    ...
    steps: [
        `함께 칠 사람을 모읍니다. ${em('매칭 만들기')}로 매칭을 열어 ${HOST_LABEL}가 되거나, ${NAV_LABEL.matchRooms}에서 비밀번호로 들어가 참가자가 됩니다. 초대를 받았다면 ${em('참가 수락')}만 누르면 됩니다.`,
        `매칭 안에서 그날 친 게임을 올립니다. ${em('게임 추가')}로 하나씩 올리거나, ${HOST_LABEL}가 ${em('자동 대진표')}로 한 번에 짤 수 있습니다.`,
        `게임마다 한 사람이 결과를 입력하면 나머지 참가자가 확인합니다. 회원 참가자 전원이 확인해야 확정되고, 틀렸다면 ${em('이의 제기')}로 다시 입력을 요청할 수 있습니다.`,
        `모든 게임이 확정되면 매칭이 종료되고, 그 경기들은 ${NAV_LABEL.myRecords}와 ${NAV_LABEL.myStats}에 반영됩니다.`,
    ],
}
...
const MATCH_ROOMS: GuideSection = {
    ...
    steps: [
        `${em(LIST_OPEN.label)} 탭에서 일시·코트·${HOST_LABEL}를 보고 매칭을 고릅니다. ${em(LIST_PAST.label)} 탭은 지난 매칭입니다.`,
        `${em('비밀번호 입장')} 표시가 있는 매칭을 열고 ${HOST_LABEL}에게 받은 비밀번호를 넣으면 곧바로 참가자가 됩니다. 정원 제한은 없습니다.`,
        `직접 열려면 ${em('매칭 만들기')}에서 방식(단식/복식)·일시·코트·비밀번호를 정하고 함께 칠 회원을 초대합니다.`,
        `카드에 ${TURN_PILLS} 같은 표시가 붙어 있으면 그 매칭에서 내가 할 일이 있다는 뜻입니다.`,
    ],
    href: MATCH_ROOMS_PATH,
    cta: `${NAV_LABEL.matchRooms} 열기`,
}
...
const MY_MATCH_ROOMS: GuideSection = {
    ...
    steps: [
        `맨 위 ${em('나를 초대한 매칭')}에서 ${em('참가 수락')}을 누르면 비밀번호 없이 참가자가 됩니다.`,
        `${em(MINE_OPEN.label)} 탭에는 결과 입력·확인이 남은 매칭이, ${em(MINE_PAST.label)} 탭에는 결과가 모두 확정된 매칭이 있습니다. 경기일이 지나도 결과가 남았으면 ${MINE_OPEN.label}입니다.`,
        `카드의 ${TURN_PILLS}·${em(ROOM_TURN_PILL.reenterResult)} 표시가 내 차례입니다. 카드를 열어 매칭 안에서 처리합니다.`,
        '결과 입력·확인·이의는 모두 매칭 안에서 합니다. 이 화면은 어느 매칭이 내 차례인지만 보여줍니다.',
    ],
    href: MY_ROOMS_PATH,
    cta: `${NAV_LABEL.myRooms} 열기`,
}
...
const PERSONAL_MATCHES: GuideSection = {
    ...
    steps: [
        ...
        `회원이 아닌 사람과 친 경기는 ${em('직접 기록')}으로 남깁니다. 회원이 한 명이라도 끼면 매칭을 만들어야 합니다. 상대의 확인이 필요하기 때문입니다.`,
        `직접 기록은 저장한 뒤 상단 ${em('결과 입력 대기')}에서 스코어를 넣는 순간 확정됩니다. 확인해 줄 상대가 없기 때문입니다.`,
        `잘못 확정한 결과는 카드의 ${em('결과 정정')}으로 되돌릴 수 있습니다. ${HOST_LABEL}가 마감한 매칭의 경기는 잠깁니다.`,
    ],
    href: PERSONAL_MATCHES_PATH,
    cta: `${NAV_LABEL.myRecords} 열기`,
}
...
const STAGES: GuideSection = {
    ...
    steps: STAGE_FLOW.map((s) => `${em(ROOM_STAGE_LABEL[s])} — ${ROOM_STAGE_HINT[s]}`),
}
...
const TERMS: GuideSection = {
    ...
    steps: [
        `${em(HOST_LABEL)} — 매칭을 연 사람. 초대·내보내기·자동 대진표·마감을 할 수 있습니다.`,
        `${em(JOINED_LABEL)} — 매칭에 들어온 상태. 게임을 올리고 결과를 확인합니다.`,
        `${em(INVITED_LABEL)} — 초대를 받았지만 아직 답하지 않은 상태.`,
        `${em(GUEST_LABEL)} — 계정 없이 이름만 올린 참가자. 결과 확인 절차가 없습니다.`,
        `${em('내 차례')} — 카드의 ${TURN_PILLS} 표시. 메뉴의 숫자가 이것을 셉니다.`,
