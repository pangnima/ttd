...
import { isSettledPersonalMatch, resolveSetWinner, type SettledPersonalMatch } from './winner'
 ...
 *
 * 결과 미확정(winner NULL) 레코드는 여기서 제외된다 — 통계·레이팅·AI 코칭이 모두
...
export function explodePersonalMatchSets(matches: PersonalMatch[]): SettledPersonalMatch[] {
    ...
    for (const m of matches) {
        if (!isSettledPersonalMatch(m)) continue  // 결과 미확정은 집계 대상이 아님
