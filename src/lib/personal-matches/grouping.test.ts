...
describe('groupByMonth', () => {
    ...
    it('멀티 게임은 게임 단위로 승패 집계하고 카드도 게임마다 1장', () => {
        // 4게임(3승 1패) 1건 + 1게임(1패) 1건 → 게임 합산 3승 2패, 그룹(박스)은 2건
        ...
        expect(june.groups.map((g) => g.sourceMatch.id)).toEqual(['a', 'b'])
        expect(idsOf(june)).toEqual(['a#0', 'a#1', 'a#2', 'a#3', 'b'])
    })
