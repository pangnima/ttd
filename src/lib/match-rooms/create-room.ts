 ...
 * `listed: false`(0082)는 매칭 리스트에 올리지 않는 비노출 방 — 비밀번호를 받지 않고 초대로만 들어온다.
 */
export type RoomScheduleInput = { durationMinutes?: number; courtCount?: number; listed?: boolean }

/**
 * 출처 기록(personal_matches / match_requests / rotation_sessions)을 매칭 룸으로 등록.
 ...
 * 생략하면 RPC 기본값(null · 1면 · 노출)이라 이 함수를 쓰는 다른 경로는 그대로 동작한다.
 *
 * 비노출 방은 비밀번호가 null이다 — RPC가 노출 방에서만 검증하고 secrets를 만든다(생성 타입이 text라
 * 빈 문자열로 보내지만 서버는 읽지 않는다). 실패해도 기록 자체는 이미 저장된 상태이므로 에러 문구에 그 사실을 함께 알린다.
 */
export async function listRecordAsRoom(
    ...
    const { data, error } = await supabase.rpc('create_match_room', {
        ...
        ...(schedule.listed === false ? { p_listed: false } : {}),
    })
    if (error) {
