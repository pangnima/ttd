 ...
 *
 * 0083이 다섯 번째 단계를 더했다 — **마감**(closed). 정산된 방을 방장이 닫은 상태로, 결과 정정·게임 추가·
 * 초대·대진 편집·기록 수정이 전부 막힌다. 정산(settled)과 갈라 두는 이유는 둘이 다른 손으로 오기 때문이다:
 * 정산은 트리거가 자동으로 세우고, 마감은 방장이 누른다(그리고 방장만 다시 연다).
 */
...
export const ROOM_STAGE_HINT: Record<RoomStage, string> = {
    ...
    closed: '방장이 매칭을 마감했습니다. 결과 정정을 포함한 모든 수정이 잠겼습니다.',
}

/** 방장이 닫았는가 — closed ⊆ settled(DB CHECK)라 정산 여부는 따로 보지 않아도 된다 */
 ...
 *  0. 방장이 닫았으면 마감 (닫힘은 정산을 전제하므로 정산보다 먼저 본다)
