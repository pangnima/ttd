 ...
 *
 * 내 경기 결과(확정)와 미확정은 `hasResult`(winner.ts) 하나로 집합 분할되고,
 ...
 * 방에 속한 행은 매칭 룸(과 매칭 리스트의 내 차례 필)이, 방 밖 행은 내 경기 결과가 그린다.
...
/**
 * 미확정 개인 경기 1행 → 버킷. `hasResult(m) === false`를 전제로 한다(확정 행은 내 경기 결과 화면 소관).
 *
