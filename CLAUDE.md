...
│   │   ├── confirmation.ts       # match_requests 행 → viewer 관점 PersonalMatchConfirmation (perspective.ts 반전 재사용) + bystanderWaitingBadge(파트너·상대2 대기 배지 문구, 0052)
...
| **결과 제안 / 확인** | 상호 확인 경기의 사후 결과 등록. `match_result_negotiations.result_status`(request_id 1:1): none → proposed(한쪽이 세트 제안, 요청자 관점으로 정규화 저장) → confirmed(상대 확인 → 양측 `personal_matches` 확정) \| disputed(이의 제기 + 사유, 양측 누구든 재제안). 제안자 본인은 확인 불가, 제안 수정만 가능. 복식 파트너·상대2는 협상을 **읽기만** 하고(0052) 대기 배지만 본다(`bystanderWaitingBadge`) |
