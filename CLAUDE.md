...
│   │   ├── me/notifications/    알림 전체 목록(최근 50) — 헤더 종 아이콘의 [전체 보기] 착지
...
│   ├── notifications/           NotificationBell(헤더 종 + 안 읽은 수, 낙관적 읽음)·Menu(최근 10)·Row·List·Link·MarkAllRead
...
│   ├── notifications/           labels.ts = 알림 21종 문구·링크 **단일 출처**(DB는 type+스냅샷만) + vitest 어휘 가드
...
/me/notifications                   알림 전체 목록(Week 71 — 최근 50, [모두 읽음]). 진입은 헤더 종 아이콘뿐(사이드바 메뉴 없음 — "할 일"은 여전히 참여 중인 매칭)
...
| 71 | 0094~0095 | **알림** — 인앱 알림(사건 13종 트리거 + 예약 5종 pg_cron), 초대 유효기간 = 매칭 종료(`invite_expired`), 결과 자동 확정(max(입력, 종료)+24h), 독촉·리마인더(초대 2h 전·시작·D-1·미입력·확정 12h 전·재입력) |
| 70 | 0092~0093 | **오픈 준비** — 이용약관·개인정보처리방침 페이지 + 동의 문구 링크(ConsentCheckbox), keepalive 크론(prod 일시정지 방지), advisor 잔여(search_path 10·anon+PUBLIC 회수 11·FK 인덱스 11·RLS initplan 57), **anon RPC 6종 시도 제한**(0093 — uid/IP 고정 윈도, `rate_limited`). 구글 동의 화면 게시·CRON_SECRET 완료 |

## 다음 할 일 (추천 순 — 1차 오픈 기준)
1. ~~구글 OAuth 동의 화면 게시~~ — **완료(2026-09-17)**: 게시 후 테스트 사용자 아닌 구글 계정으로 prod 로그인 확인. 도메인을 바꾸면 Google Auth Platform › 브랜딩의 홈·약관·방침 링크와 승인된 도메인을 함께 바꾼다.
2. ~~keepalive 크론~~ — **완료**: `CRON_SECRET` Production 적용(헤더 없는 호출 401 확인). 첫 실행은 매일 03:00 KST — Vercel › Logs에서 `/api/keepalive` 200 한 번 확인하면 끝.
3. **비밀번호 복구 경로** — 지금은 운영자 수동(`docs/history` Week 60·61). 도메인 구매 → Resend SMTP → `PASSWORD_RESET_MAIL_ENABLED = true` 한 줄.
4. ~~알림 부재~~ — **인앱 알림 완료(Week 71, 0094~0095)**. 남은 채널은 아래 「알림 채널 로드맵」. 이의 왕복 상한은 여전히 없다(재입력 독촉만).
5. **보안 잔여** — anon RPC 시도 제한은 **0093으로 닫혔다**(6종 함수 첫 줄 `throttle_hit` — 주체 uid ▸ cf-connecting-ip, 중복 확인 60/분·로그인 해석 30/분·아이디 찾기 10/분, 앱은 `RATE_LIMITED_MESSAGE`. ⚠ 서버 액션 경로는 Vercel 이그레스 IP로 묶이므로 트래픽이 커지면 service_role 클라이언트 + 앱 측 x-forwarded-for 제한으로 옮긴다) · leaked password protection은 **Pro 플랜 전용**(무료 대시보드가 거절, 2026-09-17 확인) — 앱 규칙 8자+영문+숫자+특수 + 대시보드 최소 8이 이미 흔한 유출 비밀번호를 거르므로 보류. 원하면 HIBP range API(무료·키 불필요, SHA-1 앞 5자 k-anonymity)를 `validatePassword` 서버 쪽에 붙이면 같은 효과 · advisor 잔여는 0092로 닫혔다(남은 경고는 전부 의도: 의도적 anon 6종·`match_room_secrets` 정책 0·다중 permissive·새 인덱스 미사용).

### 알림 채널 로드맵 (같은 `notifications` 테이블 위에 채널만 얹는다)
① **인앱**(Week 71 완료) — 헤더 종 + `/me/notifications`. ② **이메일** — 도메인·Resend SMTP(③ 비밀번호 복구와 같은 선행) 뒤, `notifications` INSERT에 Supabase Database Webhook → Edge Function/Vercel 라우트가 `labels.ts`와 같은 문구로 발송. ③ **PWA 웹 푸시** — `manifest.webmanifest` + 서비스 워커(next-pwa 또는 직접) → 설정 화면에서 권한 요청 → `push_subscriptions(user_id, endpoint, p256dh, auth)` 테이블 → VAPID 키 한 쌍(env, 무료) → 같은 Webhook이 `web-push`로 발송. **iPhone은 홈 화면에 추가한 PWA에서만 푸시가 온다**(iOS 16.4+) — 설치 안내가 필요하다. 실시간 구독(Realtime)은 두지 않는다 — 화면 이동마다 갱신되고 즉시성은 푸시가 맡는다.
...
- **알림 잔여**: 예약 알림은 `played_time`이 없는 레거시 방을 독촉(J1·J2)에서 뺀다 · 2일 유효창을 넘긴 due는 영영 안 나간다(의도) · 한 방에서 좌석이 겹치는 게임 N개를 연달아 입력하면 `result_proposed` N건(드롭다운 10건이 체감 상한) · 로테이션 방(세션만 있고 게임 없음)에는 결과 미입력 독촉이 없다 · 자동 확정을 끄는 방 옵션 없음.
...
## DB 스키마 현황
> 2026-09 재설계(`docs/redesign/`): 다형성 컬럼을 참가자 테이블로 정규화. 마이그레이션 0001~0095(0016부터 `supabase/migrations/*.sql`, 전체 재현 정본은 **`supabase/history/`** — 원격 히스토리 120건 사본, Week 68. 원격 적용은 MCP `apply_migration`을 dev → prod 순으로 둘 다). 원격 DB의 정의가 레포에 없으면 `execute_sql`로 읽어 마이그레이션에 편입한다 — 히스토리 밖 정의는 prod에 재현되지 않는다.
...
| `notifications` | **0094** — 알림 사건 기록 `(user_id, type, room_id·request_id·actor_user_id set null, payload jsonb, dedupe_key, created_at clock_timestamp, read_at)`. RLS: `authenticated` SELECT 본인 행만, **쓰기 정책 0**(트리거·RPC 전용). `(user_id, dedupe_key)` 부분 유니크가 예약 알림 멱등 키. payload = `room_snapshot`(playedAt·playedTime·durationMinutes·matchType·courtName·hostName) + actorName + 타입별 키 — 방·사람이 지워져도 문구가 선다(FK set null). 문구는 DB에 없다(`lib/notifications/labels.ts`). `match_result_negotiations.disputed_at`(0094 BEFORE 트리거 스탬프)이 재입력 독촉의 기준 |
| `rpc_rate_limits` | 0093 — anon RPC 시도 제한 카운터 `(bucket, subject) → window_start·hits`. 정책 0개(RPC 전용), `throttle_hit(bucket, limit, window)`이 upsert하고 한도 초과 시 `rate_limited` raise. 주체는 `throttle_subject()` = uid ▸ `cf-connecting-ip` ▸ x-forwarded-for. 호출의 1%가 하루 지난 행을 청소 |

**헬퍼**: `room_start_at(r)`·`room_end_at(r)`(0094 — KST timestamptz. `played_time` null이면 그날 전체, `duration` null이면 120분. 앱 거울 `roomStartAt/roomEndAt`(schedule.ts, vitest)와 규칙이 같아야 한다), `notify_users(ids, type, room, request, actor, payload, dedupe_key)`(행위자·탈퇴자 제외, 스냅샷 합성 — **알림을 넣는 유일한 문**), `room_joined_user_ids`·`room_reachable_user_ids`, `is_club_owner/approved_member/owner_or_officer`, `is_request_party`, `is_rotation_session_party`·`is_rotation_session_seat`, `is_room_participant`, `is_active_member` (SECURITY DEFINER — 정책식의 상호 재귀 우회)
...
- 가입·프로필(**전부 0093부터 첫 줄에서 `throttle_hit`을 거친다** — 한도를 넘으면 `rate_limited`, 앱은 `isRateLimited`로 잡아 `RATE_LIMITED_MESSAGE`. 브라우저 debounce 훅은 실패 시 열어 둔다): **`is_nickname_taken`**(0080)·**`is_email_taken`**(0081) — 가입 폼이 제출 전에 중복을 말하기 위한 판정. **둘 다 `anon` EXECUTE를 허용한다**(레포 기본 규칙의 명시적 예외) — 쓰이는 자리가 로그인 이전이고, `users_select` 정책이 `authenticated` 전용이라 브라우저의 직접 select는 에러가 아니라 **빈 결과**로 돌아와 "어떤 값이든 사용 가능"이라 거짓말을 한다. 닉네임 쪽은 0079 인덱스와 **같은 표현식·같은 집합**(`lower(btrim())` + `deleted_at is null`)을 봐야 화면과 저장이 갈리지 않는다. 이메일 쪽은 **`auth.users`를 본다** — 탈퇴는 `public.users.email`만 치환하고 auth 행은 남겨 재가입을 막으므로, "가입할 수 있나"의 권위는 그쪽이다. ⚠ 이메일 판정은 **의도적으로 user enumeration을 연다**(Week 52 결정). 되돌리려면 anon EXECUTE 회수 + 폼의 실시간 검사 제거로 족하다. **`is_login_id_taken`·`resolve_login_email`**(0085) — 아이디 중복 판정과 아이디→이메일 해석. 둘 다 anon EXECUTE(가입·로그인 화면용). `resolve_login_email`은 `auth.users.email`을 돌려주므로 **아이디→이메일 열거를 연다**(0081보다 한 단계 더) — `loginAction` 안에서만 부르고 이메일은 브라우저에 실리지 않으며 없는 아이디도 비밀번호 오류와 같은 문구이지만, curl 우회와 시도 제한 부재는 남는다. 되돌리려면 anon 회수 + 로그인 폼을 이메일 전용으로. **`find_login_id`**(0086) — 아이디 찾기. 이름+이메일이 맞으면 jsonb(`login_id`+마스킹 / `email_only` / `social`+provider), 아니면 null. anon EXECUTE. **마스킹은 함수 안에서만**(원문 비노출) 
- 알림(0094~0095): **트리거** T1 `match_room_members`(초대·수락·거절·비밀번호 입장·나감·내보내짐 — `join_match_room_as_player` upsert 전이로 수락/입장이 갈린다) · T2 `match_result_negotiations`(입력·재입력·확인·**자동 확정(uid null = 시스템)**·이의·정정 요청 — 이의/정정은 OLD proposed/confirmed로 갈린다) · T3 `match_rooms` 마감(AFTER UPDATE closed_at)·삭제(**BEFORE DELETE** — 멤버 행이 살아 있을 때) · T4 `disputed_at` 스탬프. 자동 대진표는 `create_room_lineup`·`replace_room_lineup` **말미 1회**(행 트리거면 게임 수만큼 터진다). `respond_room_invite`는 **종료 뒤 수락에 `invite_expired`**(거절은 허용). `mark_notifications_read(ids?)`(authenticated, 본인 행). **`run_notification_jobs()`**(pg_cron `*/10`, postgres 소유) — J1·J2 초대 독촉(시작 2h 전·시작) · J3 초대 만료→호스트 · J4 D-1 20:00 KST · J5 종료+3h 미입력 · J6 자동 확정 12h 전 · **J7 자동 확정**(deadline = `greatest(proposed_at, room_end_at) + 24h`, `confirmed_by := request_result_seats` 후 `settle_match_result` — 0060 BEFORE 트리거는 confirmed_by만 바뀌면 초기화하지 않는다) · J8 이의 24h 재입력 독촉. 예약은 전부 dedupe_key + **2일 유효창**(J7만 창 없음). `cleanup_notifications()`(읽은 지 30일, 03:10 KST). 잡 등록은 마이그레이션에서 `cron.unschedule` 선행(job 테이블은 데이터라 재생 시 중복)
...
| **알림 / 초대 유효기간 / 자동 확정** | **종(헤더) = "일어난 일", 사이드바 뱃지 = "내가 할 일"** — 둘을 섞지 않는다(Week 71). 알림은 `notifications` 한 테이블(채널 무관, 채널은 위에 얹는다 — 로드맵)이고 문구는 `labels.ts`가 type·스냅샷에서 만든다. **초대는 매칭 종료 시각까지만 유효** — 종료 = `room_end_at`(시각 없으면 그날 끝, 소요 시간 없으면 120분). 종료 뒤엔 수락 불가(`invite_expired`)·목록·뱃지에서 사라지고(`isInviteExpired`)·상세는 안내, 호스트에게 「응답하지 않았습니다」. status는 바꾸지 않는다(시간 해석). **결과 자동 확정** = `max(입력 시각, 매칭 종료) + 24h` 동안 이의가 없으면 시스템이 좌석 전원 확인으로 간주해 정산(확정의 세 번째 손). 되돌리기는 [결과 정정]. 입력 즉시 「확인해 주세요」, 12h 전 리마인드, 확정 시 알림 — 예약 알림은 전부 dedupe 키 1회·2일 유효창. 자동 확정과 앱 술어는 충돌하지 않는다(행이 has_result가 되어 큐에서 빠지고 `canReopenResult`는 참) |
...
- `match_requests`·`match_result_negotiations`·`rotation_session_participants`·`match_room_secrets`·`notifications` 직접 INSERT/UPDATE(RPC·트리거 전용 — 알림은 `notify_users`가 유일한 문, 읽음은 `mark_notifications_read`)
