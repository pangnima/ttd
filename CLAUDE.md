...
│   │   │   └── match-requests/   # 경기 확인 요청 허브 = 미확정 전량의 작업 큐 (내 차례/상대 대기/이의 제기 3탭 상호배타, 0061)
...
│   │   └── attendance-stats.ts
│   ├── match-requests/           # queue.ts — 미확정 경기 1행 → 작업 큐 버킷(confirmResult/enterResult/fillLineup/awaitingCounterpart + 이의 탭 전용 reenterResult/awaitingReentry, 0061) + myTurnTotal 뱃지 정의(이의 재입력 포함)·disputeTotal / tabs.ts — 허브 3탭 메타(HUB_TABS·resolveHubTab·hubTabHref, 매칭 리스트 tabs.ts 관용구) / participants.ts — 요청 좌석 규칙(0056: requiresAllMembers 방 안팎 경계, classifyPendingRequest respond/mine/awaitMembers 레인, viewerSideOf 반전 부호, acceptanceProgress 진행도, groupRotationRequests 세션 묶음) (둘 다 순수, vitest)
...
/me/match-requests → 경기 확인 요청 허브 (미확정 전량의 작업 큐 — 내 차례 기본 / ?tab=waiting 상대 대기 / ?tab=disputed 이의 제기. 세 탭은 상호배타)
...
| `match_requests` | SELECT는 `is_request_party`(요청 당사자 둘 + 복식 참가자, 0052), requester만 취소, opponent만 거절. **생성은 `create_match_request` RPC 전용**(직접 INSERT 정책 폐지 — 복식 참가자 원자적 삽입을 위해). `set_scores`는 요청 시점 원본값(빈 배열 허용). 수락은 RPC로만. 복식 파트너/상대2는 `match_request_participants`로, 결과 협상(`result_status`/`proposed_set_scores`/`proposed_by`/`proposed_at`/`dispute_reason`)은 `match_result_negotiations`로 분리(요청 상태축과 결과협상축이 별개 테이블). `court_name`은 수락 시 양측 기록에 복사(notes는 요청자만). `opponent_accepted_at`(0056) = 대표의 참여 수락 시각 — status는 나머지 참가자를 기다리느라 아직 pending일 수 있다. `rotation_session_id`·`group_seq`(0056)로 로테이션 파생 요청을 세션에 묶고, pending 중복 방지 유니크 인덱스에서 제외한다(한 세션은 같은 대표와 여러 게임을 치른다) |
...
| **작업 큐 / 내 차례** | 확인 요청 허브(`/me/match-requests`)가 미확정 전량을 담는 단일 작업 큐다. 분류는 순수 함수 `classifyPendingMatch`(`lib/match-requests/queue.ts`)가 미확정 `personal_matches` 1행을 **confirmResult(상대 제안 확인) / enterResult(내가 입력) / fillLineup(참가자 채우기) / awaitingCounterpart(상대 대기) / reenterResult(이의 · 다시 입력할 차례) / awaitingReentry(이의 · 재입력 대기)** 여섯 버킷 중 하나로 보낸다(이의 두 버킷은 0061 — disputed 판정이 좌석 폴백보다 앞이라 disputed 행은 「이의 제기」 탭에만 간다). 「내 차례」 탭 = 앞의 세 버킷 + 참여 확인(pending 요청·방 초대)이고, pending 요청 자체는 `classifyPendingRequest`가 **respond(내 좌석이 미응답) / mine(내가 보낸 요청) / awaitMembers(내 응답은 끝났고 남은 회원 대기)** 세 레인으로 가른다(0056 — awaitMembers는 취소 권한이 없어 「상대 대기」의 별도 섹션이다), 사이드바·모바일 뱃지 = `myTurnTotal(counts)` 하나다. 「상대 대기」 탭(`?tab=waiting`)은 뱃지에서 빠진다. 「이의 제기」 탭(`?tab=disputed`, 0061)은 disputed 행 전용이고 **세 탭은 상호배타**다 — 뱃지(`myTurnTotal`, reenterResult 포함) = 내 차례 탭 배지(myTurnTotal − reenterResult) + 이의 탭 배지(reenterResult). 허브 섹션은 `PendingMatchSection`이 `buildMatchGroups`로 로테이션 세션 게임을 헤더 + '게임 N'(group_seq = 입력 순) 카드로 묶는다. 조회는 `fetchMatchQueue`(React `cache()`) 단일 소스 |
