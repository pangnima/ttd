...
## DB 스키마 현황
> 2026-09 재설계(`docs/redesign/`): 다형성 컬럼을 참가자 테이블로 정규화. 마이그레이션 0001~0093(0016부터 `supabase/migrations/*.sql`, 전체 재현 정본은 **`supabase/history/`** — 원격 히스토리 120건 사본, Week 68. 원격 적용은 MCP `apply_migration`을 dev → prod 순으로 둘 다). 원격 DB의 정의가 레포에 없으면 `execute_sql`로 읽어 마이그레이션에 편입한다 — 히스토리 밖 정의는 prod에 재현되지 않는다.
