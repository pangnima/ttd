...
- **Week 40 잔여**: 미확정 로테이션 복식 방에서는 [자동 대진표]와 로테이션 빌더가 **더 이상 함께 보이지 않는다**(0071이 라인업에 `room_not_ready` 가드를 걸었다 — 버튼은 남고 저장이 막힌다. 버튼 자체를 감추는 것은 남은 일)
