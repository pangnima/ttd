...
│   ├── match-requests/           # 확인 요청 허브 (MyTurnPanel/WaitingPanel 2탭, QueueSection 섹션 껍데기, PendingMatchActions 버킷→액션 디스패처, QueueSummaryBanner 개인 결과 화면 배너, Received/SentRequestCard, RoomInviteCard 매칭 룸 초대, RequestTeamLine, 상태 뱃지)
...
│   │   └── attendance-stats.ts
│   ├── match-requests/           # queue.ts — 미확정 경기 1행 → 작업 큐 버킷(confirmResult/enterResult/fillLineup/awaitingCounterpart) + myTurnTotal 뱃지 정의 (순수, vitest)
...
| `match_request_participants` | `is_request_party(request_id)` SELECT. `{request_id, role(partner/opponent2), user_id, name, dominant_hand, ntrp_snapshot}`. 쓰기는 `create_match_request` RPC 전용 |
...
| **작업 큐 / 내 차례** | 확인 요청 허브(`/me/match-requests`)가 미확정 전량을 담는 단일 작업 큐다. 분류는 순수 함수 `classifyPendingMatch`(`lib/match-requests/queue.ts`)가 미확정 `personal_matches` 1행을 **confirmResult(상대 제안 확인) / enterResult(내가 입력) / fillLineup(참가자 채우기) / awaitingCounterpart(상대 대기)** 네 버킷 중 하나로 보낸다. 「내 차례」 탭 = 앞의 세 버킷 + 참여 확인(pending 요청·방 초대)이고, 사이드바·모바일 뱃지 = `myTurnTotal(counts)` 하나다. 「상대 대기」 탭(`?tab=waiting`)은 뱃지에서 빠진다. 조회는 `fetchMatchQueue`(React `cache()`) 단일 소스 |
