> **보관본(설계 당시 기록)** — Week 27~38의 경기 기록 도메인 재정립 계획. 이후 Week 39(매칭 룸 중심 개편)·Week 45(참여 중인 매칭 분리)·Week 67(메뉴 명칭)로 IA가 바뀌어 **아래 「목표 IA」·탭 표는 현재와 다르다**. 현행 구조는 `CLAUDE.md` 「페이지 구조」·「도메인 어휘」. 왜 그렇게 바뀌었는지는 `docs/history/`.
...
> | 4 · 허브 실 연동 | ✅ 완료 | 픽스처·`result-confirm-card` 제거, 내 차례/상대 대기 2탭 8섹션 → 0061에서 「이의 제기」 탭 추가(3탭) → 0064에서 「경기 확정 대기」 분리(4탭) → **Week 38에서 2단 탭(승인 요청[초대/경기 결과 확정/이의 신청] · 상대 승인 대기)으로 재편 — 현재 구조는 CLAUDE.md 「작업 큐 / 내 차례」가 정본이고 아래 Step 4의 탭 표는 설계 당시 기록이다** |
...
| **경기 확인 요청** `/me/match-requests` | 미확정 전량의 단일 작업 큐 (Week 38부터 2단 탭 — **승인 요청**[초대 / 경기 결과 확정 / 이의 신청] · **상대 승인 대기**. 한 행은 한 자리에만) | 요청 대기·결과 미입력·확인 대기·이의·로테이션 미입력·모집 중·룸 초대 |
...
> ⚠ 아래 탭 표는 Step 4 설계 당시(2탭 → 0061 3탭)의 기록이다. 0064의 4탭을 거쳐 **Week 38부터 2단 탭**이다:
> 최상위 「승인 요청」(내 차례 — 하위 「초대」/「경기 결과 확정」/「이의 신청」) · 「상대 승인 대기」. 섹션 1~4·9는 승인 요청 아래로,
> 5~8·10~11과 재입력 결과 확인 대기는 상대 승인 대기로 갔고, **섹션 3 「결과 입력 대기」는 허브를 떠나 개인 경기 결과 상단으로 갔다**(허브는 승인 전용).
> 현재 규칙은 CLAUDE.md 「작업 큐 / 내 차례」·`lib/match-requests/tabs.ts`가 정본이다.
