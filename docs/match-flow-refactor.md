...
**엔드투엔드 수동 시나리오** (계정 2개: `admin@admin.com / 123123` + 별도 계정)
1. 회원 상대 단식 확인 요청 → 상대 수락 → 결과 제안 → 상대 확인 → **양쪽 개인 경기 결과에 확정 카드 등장, 허브에서 사라짐**
2. 페어 고정 복식 요청 → 파트너 계정에서 협상 상태가 정확히 보이는지 (Step 11 검증)
3. 모집형 로테이션 방 등록 → 다른 계정 비밀번호 입장 → **룸 안에서** 각자 자기 기준 게임 입력 → 제안/확인 → 방장 '게임 입력 종료'
4. 매칭 리스트 3탭에서 위 방이 규칙대로 이동(진행 중 → 종료된, '내가 참여한'에는 계속 남음). `?tab=upcoming`·`?tab=garbage` 폴백
5. **미확정 상태 9종을 만들어 개인 결과 목록에 하나도 없고 허브에 전부 있는지 대조**
6. 뱃지 = 허브 '내 차례' 총건수 = 배너 숫자 3자 일치. 로테이션 방에 게임을 넣은 뒤 **뱃지가 실제로 내려가는지**(`enteredSessionIds`)
