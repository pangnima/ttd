...
- `users.ntrp`(자가선언 정적값)는 기존대로 **수동 유지**하되, 별도로 **개인 경기(personal_matches) 기반 동적 개인 레이팅**을 산출한다(§9). 클럽 NTRP·자가선언 NTRP·개인 경기 레이팅은 각각 독립된 산출물이다.

### 설계 결정 요약
| 항목 | 결정 |
|---|---|
| 자가선언 NTRP | **수동 유지** — `users.ntrp` 그대로, 경기로 자동 변동 안 함 |
