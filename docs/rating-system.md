...
- **복식 단순 모델**: 파트너 강함은 **무시**한다. 상대(팀) 평균 수준 대비 내 승패만 1:1 ELO로 반영(단식과 동일 공식).

### 9.2 상대 레이팅 공급 (fallback 체인)
개인 경기 상대 대부분이 미등록 외부 선수라 상대 레이팅이 없다. 폼에서 받은 **상대(팀) 추정 수준**(`personal_matches.opponent_ntrp`, 0023 마이그레이션)을 우선 사용하고, 없으면 다음 순서로 보강한다.
1. 저장된 `opponent_ntrp`(1.0~7.0)
2. 등록 상대 `users.ntrp`(복식은 상대1·2 평균)
3. 본인 `users.ntrp`(동급 가정)
4. `DEFAULT_RATING`(2.5)

> `users.ntrp`가 0/미설정인 경우는 위 체인에서 건너뛴다(미설정으로 간주).
