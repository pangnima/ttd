...
| `K_PROVISIONAL` | **0.20** | 잠정기(경기 수 < 임계) 최대 변동폭 |
| `K_BASE` | **0.10** | 정착 후 최대 변동폭 |
...
| 조건 | K |
|---|---|
| 클럽 경기 수 `< PROVISIONAL_THRESHOLD(10)` | `K_PROVISIONAL` = 0.20 |
| 그 이상 | `K_BASE` = 0.10 |

> K는 **각 선수의 클럽 경기 수**를 기준으로 개별 적용한다(상대가 잠정기여도 본인 경기 수로 판단).
> K=0.10일 때 동급 상대 한 판 승리 ≈ ±0.05, 이변승 ≈ +0.09(상한 근처), 강자의 예상 승 ≈ +0.01.
...
1. **이변승(단식)**: A 2.500 vs B 4.000, A가 6-4·6-4 승.
   - `E_A = 1/(1+10^(1.5)) ≈ 0.031`, `Gw=12, Gl=8 → dominance=0.2 → mf=1.10`, `K=0.10`
   - `ΔA = 0.10 × 1.10 × (1 − 0.031) ≈ +0.107` → A 2.500 → **2.607**
   - `ΔB ≈ −0.107` → B 4.000 → **3.893**
2. **강자 예상 압승**: 같은 매치에서 B가 6-1·6-2 승.
   - `Gw=12, Gl=3 → dominance=0.6 → mf=1.30`
   - `ΔB = 0.10 × 1.30 × (1 − 0.969) ≈ +0.004` (거의 불변), `ΔA ≈ −0.004`
   - → 요구사항(강자가 약자에게 압승해도 소폭 상승) 충족
3. **잠정기 신규 강자 정착**: C(실제 4.0, 클럽 2.500, 경기 0 → K=0.20) vs D 2.500, C가 6-1·6-0 승.
   - `E_C = 0.5`, `Gw=12, Gl=1 → dominance≈0.846 → mf≈1.42`
   - `ΔC = 0.20 × 1.42 × (1 − 0.5) ≈ +0.142` → C 2.500 → **2.642** (빠르게 상승)
...
- **Provisional rating**: 체스·UTR의 초기 고-K 빠른 수렴기 개념. 본 시스템은 10경기 미만 K=0.20.
