> **보관본** — Week 14~15의 레이팅 시스템 명세. §1의 "NTRP는 자가 선언 정적 값" 서술은 그 시점의 것이고, 지금은 클럽 ELO(`lib/rating/elo.ts`)·개인 NTRP(`personal-rating.ts`)·8계급 티어(`tier.ts`)가 구현돼 있다. 상수·공식은 코드(`lib/rating/`, vitest)가 정본.
...
현재 NTRP는 `users.ntrp`에 **자가 선언 정적 값**으로만 존재한다(가입 시 1.0~4.0 중 1회 선언, 이후 변경 불가). 경기를 해도 변하지 않아 실력 변화를 반영하지 못하고, 모든 클럽에서 같은 값이 노출된다.
...
- `users.ntrp`(자가선언 정적값)는 가입 시 1회 선언한 값을 **그대로 유지**하되(프로필에서 수정 불가), 별도로 **개인 경기(personal_matches) 기반 동적 개인 레이팅**을 산출한다(§9). 클럽 NTRP·자가선언 NTRP·개인 경기 레이팅은 각각 독립된 산출물이다.

### 설계 결정 요약
| 항목 | 결정 |
|---|---|
| 자가선언 NTRP | **불변** — `users.ntrp` 가입 시 1회 선언(1.0~4.0), 프로필 수정 불가, 경기로 자동 변동 안 함 |
| 개인 경기 레이팅 | **동적 산출(§9)** — 개인 경기 승패 기반, 온더플라이 계산(미저장) |
...
내부 저장은 연속 rating(1.0~7.0) 그대로 두고, **표시만 계급으로 밴딩**한다(USTA 동적 NTRP·UTR 관행과 동일). 화면에서는 숫자(`2.607`)를 노출하지 않고 **계급 배지 + 구간 내 0~99 포인트**로 표시한다. 구현: `lib/rating/tier.ts`(순수, 테스트), `components/common/tier-badge.tsx` / `tier-delta-badge.tsx`.

### 5.1 계급 구간 (폭 0.5 균등, 기본값 2.5 = 골드)
경계는 `lib/rating/tier.ts`의 `TIER_BANDS` 1곳에서 관리한다. min 포함, 다음 계급 min 미만.

| 계급 | 구간(rating) |
|---|---|
| 아이언 Iron | 1.00 ≤ r < 1.50 (하한 = MIN_RATING) |
| 브론즈 Bronze | 1.50 ≤ r < 2.00 |
| 실버 Silver | 2.00 ≤ r < 2.50 |
| 골드 Gold | 2.50 ≤ r < 3.00 (기본값 2.5 = 골드 0p, 신규/평균 진입점) |
| 플래티넘 Platinum | 3.00 ≤ r < 3.50 |
| 다이아몬드 Diamond | 3.50 ≤ r < 4.00 |
| 마스터 Master | 4.00 ≤ r < 4.50 |
| 챌린저 Challenger | r ≥ 4.50 (상한 = MAX_RATING, 종착·승급 없음) |

### 5.2 포인트 환산 (ELO 파생)
포인트는 별도 저장 없이 rating에서 파생한다 → rating 하락 시 자동 강등(강등 보호막 없음).
```
points(r) = clamp(round((r − low) / (high − low) × 100), 0, 99)
```
- 폭 0.5 기준 rating +0.005 ≈ +1p. `points = 100`은 곧 다음 계급 0p(승급)으로 표현(상한 99 클램프).
- 예: 2.5 → 골드 0p, 2.75 → 골드 50p, 2.99 → 골드 98p.
- 챌린저는 종착(`pointsToPromote = 0`).

### 5.3 노출 지점
- **멤버 목록 / 멤버 프로필 헤더 / 클럽 멤버 미리보기**: 계급 배지(+포인트), 잠정이면 "잠정" 병기. 통합 NTRP는 별도 라벨로 유지.
- **클럽 대시보드 "레이팅 랭킹" 카드**: rating 내림차순 정렬은 유지, 표시는 계급+포인트.
- **대진표 상세(확정)**: 경기 전/후 rating → `TierDeltaBadge`로 포인트 변동(▲13p / ▼13p) 또는 계급 변동(▲골드 승급 / ▼실버 강등). 하단 "레이팅 변동" 요약도 동일.
- **프로필 클럽 레이팅 추세**: 헤더에 현재 계급+포인트와 시작→현재 변동, 스파크라인은 raw rating 시계열 형태 유지.
...
## 9. 개인 경기 레이팅 (Personal Match Rating)

`me/personal-matches`에 기록되는 **개인 경기(클럽 외부 포함) 승패**로 본인의 개인 레이팅을 산출한다. 클럽 NTRP와 동일한 ELO 스케일·티어 표시(§5)를 **그대로 차용**하되, 다음 점이 다르다.

### 9.1 핵심 차이
- **온더플라이 계산**: 별도 테이블에 저장하지 않는다. 조회 시점에 본인 개인 경기를 순수함수로 재생(replay)해 산출한다. 경기 추가/수정/삭제가 즉시 반영되며 재계산 트리거·owner 버튼이 필요 없다.
- **'나' 한 명만 추적**: 클럽 ELO처럼 모든 참가자 풀을 굴리지 않고, 본인의 레이팅 시계열만 순차 갱신한다.
- **복식 팀-대-팀 블렌드**: 기대승률은 **'내 팀' 사이드 레이팅** vs **상대팀 평균**으로 계산한다. 내 팀 사이드 = `avg(내 현재 레이팅, 파트너 강도)`. 변동 delta는 팀이 아니라 **내 개인 레이팅**에 적용한다(단식은 내 레이팅 그대로). 0024 마이그레이션으로 선수별 NTRP(`partner_ntrp`/`opponent2_ntrp`)를 저장한다.

### 9.2 레이팅 공급 (fallback 체인)
개인 경기 상대 대부분이 미등록 외부 선수라 레이팅이 없다. 폼에서 받은 **선수별 추정 NTRP**를 우선 사용하고, 없으면 보강한다.

**상대팀 레이팅** (`resolveOpponentRating`):
1. 저장된 NTRP(단식 `opponent_ntrp` / 복식 `opponent_ntrp`·`opponent2_ntrp` 평균)
2. 등록 상대 `users.ntrp`(복식은 상대1·2 평균)
3. 본인 `users.ntrp`(동급 가정)
4. `DEFAULT_RATING`(2.5)

**내 팀 사이드 레이팅** (`resolveSelfSideRating`, 복식 전용 — 단식은 내 레이팅):
- `avg(내 현재 레이팅, 파트너 강도)`. 파트너 강도: ① 저장된 `partner_ntrp` → ② 파트너 회원 `users.ntrp` → ③ 내 현재 레이팅(동급=무영향).

> `users.ntrp`가 0/미설정인 경우는 위 체인에서 건너뛴다(미설정으로 간주). 파트너 정보가 전혀 없으면 내 팀 사이드 = 내 레이팅 → 기존 동작과 동일.

### 9.3 시작값·정렬·잠정기
- **시작 레이팅**: 본인 `users.ntrp`(>0)면 그 값, 없으면 `DEFAULT_RATING`(2.5).
- **정렬키**: `(played_at, played_time, id)` 오름차순 — 입력 순서와 무관하게 결정적.
- **K 계수·잠정기·마진·경계 클램프**: 클럽 엔진과 동일 상수(§2.1) 재사용. `matchesPlayed < PROVISIONAL_THRESHOLD(10)`이면 잠정.

### 9.4 구현
- 순수 코어: `lib/rating/personal-rating.ts` — `replayPersonalRatings`, `resolveOpponentRating`. 클럽 엔진(`elo.ts`)의 스칼라 primitive(`expectedScore`/`pickK`/`marginFactor`/`computeMatchDelta`)를 **재사용**(코어 엔진 무수정). 단위 테스트 `personal-rating.test.ts`.
- 입력: `personal-match-form.tsx`의 "상대(팀) 수준" 셀렉트(등록 회원 선택 시 ntrp 자동 프리필).
- 노출: `personal-rating-trend-card.tsx`(클럽 추세 카드 패턴 복제), 본인 프로필 개인/통합 scope의 `SelfAnalyticsSection`에서 렌더.
