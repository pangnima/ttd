...
stateDiagram-v2
    ...
    직접기록 --> 결과미확정: 세트 없이 저장 (winner=NULL)
    직접기록 --> 결과확정: 세트 등록 후 저장 (winner 파생)
    ...
    결과미확정 --> 통계집계제외: (불변식) 통계·레이팅·AI코칭 모두 winner=NULL 제외
...
- 결과 미확정(`winner = NULL`) 경기는 통계·레이팅·AI 코칭 집계에서 전부 제외된다.
