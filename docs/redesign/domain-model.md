...
**복식 대표 확인자 규칙** (`resolveConfirmRep`, 0038): 상대팀 2명 중 회원인 사람이 `opponent_user_id`(대표)가 되고, 나머지(파트너 포함)는 회원이어도 그들의 `personal_matches`에는 레코드가 생기지 않는다 — "참가자 4명 중 실제로 기록을 갖는 사람은 요청자+대표 확인자 2명뿐"이라는 비대칭 규칙. 경기 방 게임(0049)만 이 비대칭을 깨고 회원 4명 모두 기록을 갖는다. **→ 이후 변경**: 0053이 방 밖 복식도 4명 전원에게 관점 행을 만들고, 0059가 협상 권한을 좌석 넷으로, 0060이 확정을 **좌석 전원 만장일치**(`confirmed_by`)로 바꿨다 — '대표'는 이제 `opponent_user_id` 슬롯 결정 규칙일 뿐이다.
...
- 복식 상호확인에서 파트너·상대2가 회원이어도 그들의 `personal_matches`에는 레코드가 생성되지 않는다 — 기록은 요청자와 대표 확인자 2명만 갖는다. **예외: 경기 방 게임**(`room_id` 있음, 0049)은 회원 참가자 전원에게 각자 관점의 행을 만든다 — 같이 친 사람이 방 명단으로 이미 확인돼 있기 때문이다. 결과 협상은 그래도 요청자·대표 2명이 하고, 확정 시 4행이 함께 갱신된다. **→ 이후 변경(0053·0059·0060)**: 방 안팎 모두 회원 참가자 전원에게 관점 행이 생기고, 좌석 넷 전원이 협상하며, 확정은 제안자를 포함한 활성 회원 좌석 전원의 확인(`confirmed_by`)으로만 이뤄진다.
