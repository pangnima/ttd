...
**`scripts/e2e-cleanup.sql`** 하나다 — 태그(코트명·메모 `E2E-%`)로 잡은 방·요청·세션·기록·**알림**(0094 — 방을 지우면 room_id만 null이 되고 남으므로 payload의 코트명으로 잡는다)과 `@e2e.test` 계정을 한 트랜잭션으로 지우고 마지막 SELECT가 전부 0이어야 한다. **dev(테스트 서버)에서만** 돌린다: Supabase MCP `execute_sql`에 파일 전체를 그대로 보낸다.
